...
const exportResultMonth1 = () => {
  ...
  const reconQuery = 'SELECT T1.Database_ID, Lease_Description, SUM(BR_Current_Month_Cash - BR_Current_Month_Cash_Client) total, SUM(Total_Current_Month_Cash - Total_Current_Month_Cash_Client) total2 FROM `LQ 1` AS T1  INNER JOIN `CD 1` AS T2 ON T1.Database_ID = T2.Database_ID GROUP BY Database_ID'
